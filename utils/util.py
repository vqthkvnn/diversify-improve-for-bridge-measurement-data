...
def act_param_init(args):
    ...
    args.select_channel = {'emg': np.arange(5)}
    # args.select_channel = {'emg': np.arange(8)}
    args.hz_list = {'emg': 1000}
    classes = []
    [classes.append([x]) for x in np.arange(0, 16, 1, dtype=int)]
    args.act_people = {'emg': classes}
    # args.act_people = {'emg': [[i*9+j for j in range(9)]for i in range(4)]}
    tmp = {'emg': ((5, 1, 8000), 16, 10), 'labdata': ((4, 1, 200), 4, 10)}
